function varargout = setup_root_profile_M(varargin)
...
% Last Modified by GUIDE v2.5 11-Oct-2021 14:38:58
...
% load 'WorkFol.mat' 'workingpath'
% 
% Tras_FileName=[workingpath '/Temps/temp_variable.mat'];

% Tras_FileName='transfer.mat';

load('.\Temps\temporary.mat', 'config_path');

Config_folder=config_path;

file_name   = 'transfer.mat';
Tras_FileName    =...
    fullfile(fullfile(Config_folder, 'YearFr'), file_name);

load (Tras_FileName, 'opt_root')
...
% Load table and information for observed option---------------------------
    load (Tras_FileName, 'num_root', 'dat_root')
    dat_length = length(dat_root(:,1));
    %root_num1 = str2num(num_root);
    ...
    if dat_length > root_num1
        dat_root = dat_root(1:root_num1,:);
    elseif dat_length < root_num1
        dat_root_add = zeros(root_num1-dat_length,2);
...
% Load table and information for equation option---------------------------
    load (Tras_FileName, 'set_para_root', 'set_root_func')
...
% Load litter depth
    load (Tras_FileName, 'litter_depth')
    set(handles.txt_litterDepth1,'String',litter_depth);    
...
if pathname == 0 %if the user pressed cancelled, then we exit this callback
    ...
    if length(root_data_table(:,1)) ~= num_root
        numstr=num2str(length(root_data_table(:,1)));
        notice = ['Number of root layer (',num2str(num_root),') must be equal to data in imported file (',numstr,')'];
...
litter_depth_str = get(handles.txt_litterDepth1,'String');    
litter_depth     = str2num(litter_depth_str);
if opt_root == 1
...
save (Tras_FileName,...
        'dat_root', 'opt_root', 'set_para_root', 'set_root_func', 'litter_depth', 'num_root_eqn', '-append')
...
Nrootcut            = cell2mat(para_root_eqn(4,2));


if (Nrootcut == double(uint64(Nrootcut))) % if Integer
    
else % Not integer
    msgbox('# of root must be integer','MLCan Error','Warn');    
    return
end
...
function txt_litterDepth1_Callback(hObject, eventdata, handles)
% hObject    handle to txt_litterDepth1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of txt_litterDepth1 as text
%        str2double(get(hObject,'String')) returns contents of txt_litterDepth1 as a double


% --- Executes during object creation, after setting all properties.
function txt_litterDepth1_CreateFcn(hObject, eventdata, handles)
% hObject    handle to txt_litterDepth1 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
...
% --- Executes on button press in root_rad_obs.
function root_rad_obs_Callback(hObject, eventdata, handles)
% hObject    handle to root_rad_obs (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of root_rad_obs


% --- Executes on button press in root_rad_eqn.
function root_rad_eqn_Callback(hObject, eventdata, handles)
% hObject    handle to root_rad_eqn (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hint: get(hObject,'Value') returns toggle state of root_rad_eqn
