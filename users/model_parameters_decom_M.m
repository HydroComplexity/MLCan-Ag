function varargout = model_parameters_decom_M(varargin)
...
load ('./Temps/temp_model.mat', 'dat_decom', 'dat_decom_litter', 'Start_Y')
...
if pathname == 0 %if the user pressed cancelled, then we exit this callback
    ...
    if isstr(num_root)
        num_root=str2num(num_root);
    end
    if length(DEC_data_table(:,1)) ~= (num_root+1)
        numstr=num2str(length(DEC_data_table(:,1)));
        notice = ['Number of parameters (',num_root,') must be equal to data in imported file (',numstr,')'];
        ...
        if length(DEC_data_table(1,:)) ~= 4
            ...
            load (fullpath_Py, 'dat_root', 'litter_depth');
            
            if (sum(DEC_data_table(:,1) - [litter_depth; dat_root(:,1)]) > 0.001 || sum(DEC_data_table(:,1) - [litter_depth; dat_root(:,1)]) < - 0.001)
...
save './Temps/temp_model.mat' 'dat_decom' 'dat_decom_litter' -append
