function varargout = setup_LAD_profile_M(varargin)
...
% Temp_FileName=[workingpath '/Temps/temp_variable.mat'];




% load ('transfer.mat', 'working_year',...
%     'working_spec');
...
file_nameT   = 'transfer.mat';
Tras_FileName    =...
    fullfile(fullfile(Config_folder, 'YearFr'), file_nameT);

load (Tras_FileName, 'dat_LAD', 'num_LAD')

dat_length = length(dat_LAD(:,1));

if isstr(num_LAD) == 1
    LAD_num1 = str2num(num_LAD);
else
    LAD_num1 = (num_LAD);
end

if dat_length > LAD_num1
    dat_LAD = dat_LAD(1:LAD_num1,:);
else
	dat_LAD_add = zeros(LAD_num1-dat_length,2);
...
if pathname == 0 %if the user pressed cancelled, then we exit this callback
    ...
    if length(LAD_data_table(:,1)) ~= num_LAD
        numstr=num2str(length(LAD_data_table(:,1)));
        notice = ['Number of LAD (',num2str(num_LAD),') must be equal to data in imported file (',numstr,')'];
...
save (Tras_FileName, 'dat_LAD', '-append')
