...
%*************************************************************************
    ...
    % VARIABLES structure
    Ta1 = VARIABLES.CANOPY.TAz(1, si);                                          %[C] Temperature in the Atmosphere first layer
    ...
    psis1 = VARIABLES.SOIL.smp(1, si);                                          %[mm] Soil water potential first layer
    TC1 = VARIABLES.SOIL.TKsoil(1, si);                                         %[W / m / K] Thermal Conductivity firs soil layer    
    Ts1 = VARIABLES.SOIL.Ts(1, si);                                             %[C] Temperature in the Soil fist layer  
    ...
    rhosn = VARIABLES.SOIL.rhosn(si);                                           %[kg / m^3] Snow Density  
    zsn = VARIABLES.SOIL.zsn(si);                                               %[mm] Snow Depth 
    Tsl =  VARIABLES.SOIL.Tsl(si);                                              %[C] Top Soil Temperature 
    Tlprev = VARIABLES.SOIL.Tlprev(si);                                         %[C] Previous time step snow temperature 
...
if indexss     
    ...
    VARIABLES.SOIL.LEl(si) = LEl;              % Latent Heat from Snow-Litter Pack [W/m2]
    VARIABLES.SOIL.LEs(si) = LEs;              % Latent Heat from The Soil [W/m2]    
    VARIABLES.SOIL.Hl(si) = Hl;                % Sensible heat from Snow-Litter pack [W/m2]  
    VARIABLES.SOIL.Hs(si) = Hs;                % Sensible heat from Snow-Litter pack [W/m2]  
    
    VARIABLES.SOIL.RHl(si) = nan;              % Relative Humidity in snow-litter pack.
    VARIABLES.SOIL.Tli(si) = Tli;              % Temperature Snow-Litter pack [C]  
    VARIABLES.SOIL.Tsl(si) = Tsl;              % Tempeature in the Soil-Litter Interface
    VARIABLES.SOIL.Gsl(si) = Gsl;              % Ground Heat Flux into the Soil - Snow-Litter Boundary from The Snow-Litter Pack [W/m2]
    VARIABLES.SOIL.Gs(si) = Gs;                % Ground Heat Flux into the Soil from the Soil- Snow-Litter Pack  Boundary [W/m2]
    VARIABLES.SOIL.psili(si) = nan;            % Soil Water Potential in the snow-Litter Pack [converted from m to mm]  
    VARIABLES.SOIL.psil_MPa(si) = nan;         % Soil Water Potential in the snow-Litter Pack [MPa]
    VARIABLES.SOIL.remain(si) = remain;        % Total Error in the Energy Balance
    
    VARIABLES.SOIL.deltawice(si) = deltawice;  % Delta of ice needed to change phase in the snow-litter pack
    VARIABLES.SOIL.dH(si) = dH;                % Delta of Energy Melting/Fusion 
    VARIABLES.SOIL.dS(si) = dS;                % Delta of Energy in Time 
    
    VARIABLES.SOIL.case1sl(si) = nan;          % Case 
