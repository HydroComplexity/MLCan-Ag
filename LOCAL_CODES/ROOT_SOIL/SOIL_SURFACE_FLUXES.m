function [Hs, LEs, Gs, RH, Ts, remain, VARIABLES] = SOIL_SURFACE_FLUXES(VARIABLES, VERTSTRUC, PARAMS, CONSTANTS, SWITCHES,FORCING, si)
...
%*************************************************************************
    % VARIABLES
    Ta1 = VARIABLES.CANOPY.TAz(1, si);                                          % [C] Atmospheric Temperature first layer
    ...
    volliq1 = VARIABLES.SOIL.volliq(1, si);                                     % [] Volumetric water content
    psis1 = VARIABLES.SOIL.smp(1, si);                                          % [MPa] Soil water potential of top layer
...
%Gs = Rabs - LEs - Hs;
 ...
 %    remain = Rnrad_soil - Hs - LEs - Gs;  
    ...
    VARIABLES.SOIL.LEl(si) = 0;                                                 % Latent Heat from Snow-Litter Pack [W/m2]
    VARIABLES.SOIL.LEs(si) = LEs;                                               % Latent Heat from The Soil [W/m2]    
    VARIABLES.SOIL.Hl(si) = 0;                                                  % Sensible heat from Snow-Litter pack [W/m2]  
    ...
    VARIABLES.SOIL.Tli(si) = Ts;                                                % Temperature Snow-Litter pack [C]  
    VARIABLES.SOIL.Tsl = Ts;                                                % Tempeature in the Soil-Litter Interface [C]
    VARIABLES.SOIL.Gsl(si) = 0;                                                 % Ground Heat Flux into the Soil - Snow-Litter Boundary from The Snow-Litter Pack [W/m2]
    ...
    VARIABLES.SOIL.remain(si) = remain;                                         % Total Error in the Soil Energy Balance [W/m2]
    
    VARIABLES.SOIL.deltawice(si) = 0;                                           % Delta of ice needed for change of phase in the snow-litter pack [W/m2]
    VARIABLES.SOIL.dH(si) = 0;                                                  % Delta of Energy [W/m2]
    VARIABLES.SOIL.dS(si) = 0;                                                  % Delta of Energy [W/m2]
