...
for ii=1:1:nspecies
    [krad(:,ii),kax(:,ii)] = conductivities(nl_soil,zmm,dzmm, znsmm,rootfr(:,ii),...
        roottr(:,ii),rpp(:,ii),smp(:,ii),wliq(:,ii),effporsl,...
...
while (max(converge) > 0.1) %1e-20
    ...
    if plants == 1
        for ii=1:1:nspecies
            ...
            [rpp(:,ii)] = rootmodel(nl_soil,nl_root(ii),zmm,etr(ii),smp(:,ii),krad(:,ii),kax(:,ii));
    ...
    %    pthr,nspecies, type);
    for ii=1:1:nspecies
        [dwat(:,ii),smp(:,ii),kboundary,klayer,ft,fb,qlayer,layeruptake,...
            layeruptake_all(:,ii),mberrormm,type,hor_drainage,hor_drainage_lay,...
            flux_Ss] ...
            = soilmodel(nl_soil,dtime,effporsl,phi0,bsw,hksati,zmm',...
            dzmm',zimm',wliq(:,ii),plants,rpp(:,ii),krad(:,ii),...
            pthr(ii),nspecies, type, n, alpha, thetar,vanGen);
    end
    ...
    if cnt==1
        ...
        for ii=1:1:nspecies
            converge(1,ii) =(abs(((abs(etr(ii)-sum(krad(:,ii).*(smp(:,ii)-rpp(:,ii))))) - difprev(1,ii))))/difprev(1,ii);
        end
    end
    for ii=1:1:nspecies
        difprev(1,ii) = abs(etr(ii)-sum(krad(:,ii).*(smp(:,ii)-rpp(:,ii))));
    end
    %*******************************************************************
    %      end
