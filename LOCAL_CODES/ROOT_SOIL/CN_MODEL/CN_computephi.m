...
function [phi, PHI, MIN_net, IMM_net, MIN_gross, IMM_gross, Nreg, DECl, DECh] = CN_computephi (VARIABLES, PARAMS, SWITCHES, fSd, fTd, phi, ADD, CNa, si) 
...
CNl = VARIABLES.CNl(:,si);%       CNl = carbon/nitrogen ratio of litter pool [gC / gN]
...
%            PHI(ii) = phi*fTd(ii)*fSd(ii)*Cb(ii)*(kh(ii)*Ch(ii)*(1./CNh - (1-rr)./CNb(ii)) + kl(ii)*Cl(ii)*(1./CNl(ii) - rh(ii)./CNh - (1-rh(ii)-rr)./CNb(ii)));  
            PHI(ii) = phi(ii)*fTd(ii)*fSd(ii)*Cb(ii)*(kh(ii)*Ch(ii)*(1./CNh - (1-rr)./CNb(ii)) + kl(ii)*Cl(ii)*(1./CNl(ii) - rh(ii)./CNh - (1-rh(ii)-rr)./CNb(ii)));
% Dongkook Woo - Edit end 
