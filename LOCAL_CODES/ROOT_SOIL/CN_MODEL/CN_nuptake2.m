function    [VARIABLES, SUP_amm_pas_all_m2, SUP_nit_pas_all_m2, SUP_amm_act_all_m2, SUP_nit_act_all_m2, ...
    ...
    CN_nuptake (VARIABLES, PARAMS, SWITCHES, VERTSTRUC, FORCING,...
    STORAGE, CONSTANTS, a_Nit, a_Amm, sm, layeruptake_all, rootfr)
...
%     dz_mm = [VARIABLES.SOIL.litterthickness(si)*1000 ; VERTSTRUC.dzsmm];%       dz_mm = grid depth [mm]
else
    nl_soil = PARAMS.nl_soil;%       nl_soil = # soil layers        
...
DEM = VARIABLES.CANOPY.N_Demand.*86400/dtime;
...
%     
    ...
    for i=1:1:nspecies
        Nit_ku(:,i)=(a_Nit./(sm(:,i))).*F(i).*((sm(:,i)./n).^d(i)); % [m/d]
        Amm_ku(:,i)=(a_Amm./(sm(:,i))).*F(i).*((sm(:,i)./n).^d(i)); % [m/d]
    end
%     
...
%% Rohit Nandan (to balance the size of passive and active
    if size(SUP_nit_passive) ~= size(SUP_nit)
        if size(SUP_nit_passive)<size(SUP_nit)
            for ssize=1:(size(SUP_nit)-size(SUP_nit_passive))
                SUP_nit_passive=[SUP_nit_passive, SUP_nit_passive(end)];
            end
        elseif size(SUP_nit_active)<size(SUP_nit)
            for ssize=1:(size(SUP_nit)-size(SUP_nit_active))
                SUP_nit_active=[SUP_nit_active, SUP_nit_active(end)];
            end
        end
    end

    if size(SUP_amm_passive) ~= size(SUP_amm)
        if size(SUP_amm_passive)<size(SUP_amm)
            for ssize=1:(size(SUP_amm)-size(SUP_amm_passive))
                SUP_amm_passive=[SUP_amm_passive, SUP_amm_passive(end)];
            end
        elseif size(SUP_amm_active)<size(SUP_amm)
            for ssize=1:(size(SUP_amm)-size(SUP_amm_active))
                SUP_amm_active=[SUP_amm_active, SUP_amm_active(end)];
            end
        end
    end
...
%     disp(size(SUP_amm))
%     disp('other')
%     disp(size(SUP_amm_active))
%     disp(size(SUP_amm_passive))
    
    
    %%

    % N Uptake from soil = passive upatke + active uptake
    SUP_nit=sum(SUP_nit_passive+SUP_nit_active,2); % [g/m2/d]
