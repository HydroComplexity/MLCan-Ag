function [VARIABLES] = CN_fertilizer (VARIABLES,PARAMS,CONSTANTS,VERTSTRUC, si)
...
N_Fert_DOY  = PARAMS.CN.N_Fert_DOY(si); % [DOY]
N_Fert_amm  = PARAMS.CN.N_Fert_amm(si); % [gN/m2]
N_Fert_nit  = PARAMS.CN.N_Fert_nit(si); % [gN/m2]
N_Fert_urea = PARAMS.CN.N_Fert_urea(si); % [gN/m2]
VolAmm      = PARAMS.CN.VolAmm;
VolUrea     = PARAMS.CN.VolUrea;

Nit=VARIABLES.Nit(:,si); % Nitrate concentration in soil [gN / m^3]
Amm=VARIABLES.Amm(:,si); % Ammonium concentration in soil [gN / m^3]
...
VARIABLES.Nit(:,si)=Nit;
VARIABLES.Amm(:,si)=Amm;
