...
if N_denit == 0
    Denitrif = zeros(nl_soil,nspecies);
    Total_N20 = zeros(nl_soil,nspecies);
    N2O_nit = zeros(nl_soil,nspecies);
...
elseif N_denit == 1
    ...
    FdNO3=real(1.15.*(((Nit./BulkDensity*(1000000/1))).^0.57)); % [ugN/g/d]
    % PCO2 [g/m3] ./ Bulkdensity [g/m3] .* g to ug [1g = 1000000ug]
    FdCO2=real(0.1.*(((PCO2./BulkDensity*(1000000/1))).^1.3)); % [ugN/g/d]
    
    %Mmin=ones(size(dz,1),1).*0.113;
    Mmin=ones(nl_soil,nspecies).*0.113;
