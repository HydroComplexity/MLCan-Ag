...
CNl_ini = VARIABLES.CNl(:,si);%       Initial CN in litter 

%DECLARE MATRICES 
ADD_r=zeros(nl_soil,1);      % Addition of litter from the below-ground
ADD_l=zeros(nl_soil,1);      % Addition of litter from the above-ground
...
%     ADDa = TBMla(:,timestep);                 % Total biomass Drop in this time step as litter from above [gr / m^2]
    ADDa = VARIABLES.CANOPY.DeathRate(si);
...
elseif SWITCHES.CN.NupRootBiomass == 0
%     ADDa = TBMla(:,timestep);                 % Total biomass Drop in this time step as litter from above [gr / m^2]
    ADDa = VARIABLES.CANOPY.DeathRate(si); %% Rohit
    
    ADDa = ADDa * 86400/dtime;                % Convert from [gr/m^2/dtime s] to [gr/m^2/d]
    % Below ground
%     ADDb = TBMrb(:,timestep);                 % Total biomass Drop in this time step as litter from above [gr / m^2]
    ADDb = VARIABLES.CANOPY.RootDeathRate(si); %% Rohit
...
% for jj=1:1:nspecies
    ...
    if SWITCHES.CN.NupRootBiomass == 1
        CNadd = CNveg{si}(timestep);
    elseif SWITCHES.CN.NupRootBiomass == 0
        CNadd = CNabove(si,timestep)'; 
        CNbadd = CNbelow(si,timestep)';
    end
    % Dongkook Woo - Edit End
    % add litter from below ground
    ADD_r=ADDb.*rootfr(:,si)./(dz_mm/1000); 
    % add litter above ground is distributed uniformly in first layers     
    ADD_l=zeros(nl_soil,1); 
    ADD_l(1)= ADDa/(dz_mm(1)/1000);
    % Supersposition of litter from both
    ADDtotal=ADD_r+ADD_l;
% end
...
if SWITCHES.CN.Bioturbation
    ...
    ADD_bio = VARIABLES.Cl_bio_in(:,si);   % Addition of litter from bioturbation source  [gr/m3/d]
    CNa_bio = VARIABLES.CN_bio_in(:,si);   % CN ratio in the bioturbation source
    OUT_bio = VARIABLES.Cl_bio_out(:,si);  % Extraction of litter from bioturbation source   [gr/m3/d]
    CNo_bio = VARIABLES.CN_bio_out(:,si);        % CN ration in the layer where is the out in bioturbation
