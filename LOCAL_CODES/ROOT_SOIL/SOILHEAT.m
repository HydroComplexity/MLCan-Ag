function [VARIABLES] = SOILHEAT (Hg, VARIABLES, VERTSTRUC, PARAMS, CONSTANTS, si)
...
%VARIABLES
volliq = VARIABLES.SOIL.volliq(:, si);                                            % [] Volumetric Liquid Water Content
volice = VARIABLES.SOIL.volice(:, si);                                            % [] Volumetric Ice Water Content
Ts = VARIABLES.SOIL.Ts(:, si);                                                    % [C] Temperature Soils
...
% STORE IN VARIABLES
VARIABLES.SOIL.Gnew(:, si) = [Gnew ; 0];                                        % [W / m^2] Heat Fluxes in the Soil
VARIABLES.SOIL.Ts(:, si) = Ts_new;                                              % [C] Soil Temperature
VARIABLES.SOIL.cpv(:, si) = cpv;                                                % [J / m^3 / K] Volumetric Heat Capacity of Soil
VARIABLES.SOIL.TKsoil(:, si) = TKsoil;                                          % [W / m / K] Thermal Conductivity Soil
VARIABLES.SOIL.TKsol(:, si) = TK_sol;                                           % [W / m / K] Thermal Conductivity of Mineral Solids
