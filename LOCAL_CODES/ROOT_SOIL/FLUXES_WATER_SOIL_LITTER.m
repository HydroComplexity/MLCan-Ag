...
%========================================================================
    ...
    volliqli = VARIABLES.SOIL.volliqli(si);                                     % [] Volumetric Water Content Snow-Litter Pack
    voliceli = VARIABLES.SOIL.voliceli(si);                                     % [] Volumetric Ice Content Snow-Litter Pack
    ...
    Tli = VARIABLES.SOIL.Tli(si);                                               % [C]Temperature of Snow-Litter pack
    ...
    case1sl = VARIABLES.SOIL.case1sl(si);                                       % Index that defines whether is treated as snow or as a litter
...
%    zliqsl_new = max(0,zliqsl_new);                                               %[mm liq]
    ...
    % vols
    VARIABLES.SOIL.volliqli(si) = zliqsl_new/dzlit_mm;                          % [] Volumetric Water Litter Content     
    VARIABLES.SOIL.voliceli(si) = zicesl_new/dzlit_mm;                          % [] Volumtetric Ice Litter Content     

    VARIABLES.SOIL.volliqsn(si) = zliqsl_new/zsn;                               % [] Volumetric Liquid Water Content in Snow      
    VARIABLES.SOIL.volicesn(si) = zicesl_new/zsn;                               % [] Volumetric Ice Water Content in Snow         
    VARIABLES.SOIL.voltotsn(si) = (zliqsl_new + zicesl_new)/zsn;                % [] Total Volumetric Ice and Water Snow Content         
    VARIABLES.SOIL.voltotli(si) = (zliqsl_new + zicesl_new)/dzlit_mm;           % [] Total Volumetric Ice and Water Litter Content 
    ...
    %
    VARIABLES.SOIL.Esl_ice(si) = Esl_ice;                                       %[mm/s] Sublimation from ice in the snow
    VARIABLES.SOIL.Esl_liq(si) = Esl_liq;                                       %[mm/s] Evaporation from snow (assumed zero)
