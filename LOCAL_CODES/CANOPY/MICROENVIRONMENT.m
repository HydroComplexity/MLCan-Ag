function [CAz, EAz, TAz] = MICROENVIRONMENT(VARIABLES, VERTSTRUC, PARAMS, CONSTANTS, si)
...
%*************************************************************************
% VARIABLES
CAz = VARIABLES.CANOPY.CAz(:, si);                                            % [umol/mol] Atmospheric Concentration of CO2, for all layers
TAz = VARIABLES.CANOPY.TAz(:, si);                                            % [C] Air Temperature, for all layers
EAz = VARIABLES.CANOPY.EAz(:, si);                                            % [kPa] Vapor Pressure Air, for all layers
PAz = VARIABLES.CANOPY.PAz;                                            % [kPa] Pressure Air, for all layers
Km = VARIABLES.CANOPY.Km;                                              % Momentum Diffusivity Constant
An_sun = VARIABLES.CANOPY.An_sun(si);                                      % [umol CO2/ m^2 leaf / s] Photosynthetic minus Leaf Respiration flux from canopy sunlit
An_shade = VARIABLES.CANOPY.An_shade(si);                                  % [umol CO2/ m^2 leaf / s] Photosynthetic minus Leaf Respiration flux from canopy shade
LE_sun = VARIABLES.CANOPY.LE_sun(si);                                      % [W / m^2 leaf] LE flux from canopy sunlit
LE_shade = VARIABLES.CANOPY.LE_shade(si);                                  % [W / m^2 leaf] LE flux from canopy shade
H_sun = VARIABLES.CANOPY.H_sun(si);                                        % [W / m^2 leaf] H flux from canopy shade
H_shade = VARIABLES.CANOPY.H_shade(si);                                    % [W / m^2 leaf] H flux from canopy shade
...
Fc_soil = VARIABLES.SOIL.Fc_soil(si);                                      % [m^2 leaf / m^2 ground] CO2 flux from soil
LE_soil = VARIABLES.SOIL.LE_soil(si);                                      % [m^2 leaf / m^2 ground] LE flux from soil
H_soil = VARIABLES.SOIL.H_soil(si);                                        % [m^2 leaf / m^2 ground] H flux from soil

% VERTSTRUC
znc = VERTSTRUC.znc;                                                   % [m] Height of canopy levels
dzc = VERTSTRUC.dzc;                                                   % [m] Tickness of layers in canopy
fLAIz = VERTSTRUC.fLAIz(si);                                               % [] Fraction of total LAI for each species at each canopy layer
