function [Ph, An, Ci, gsv, Tl, Tl_up, Tl_lo, LE, dHcan, TR, Evap_mm, H, psil, fsvg, fsvm, Ch2o_mm, gbv, gbh, ...
...
%*************************************************************************
Tl = nan(size(dryfrac));
Tl_up = nan(size(dryfrac));
Tl_lo = nan(size(dryfrac));
...
while(~converged)
    ...
    %% Rohit start
    if (canstorheat)
        [Tl_dry_up] = LEB_QUARTIC_DRY_CHEAT_up (VARIABLES, PARAMS, VERTSTRUC, CONSTANTS, sunlit,cntspecies);
    else
        [Tl_dry_up] = LEB_QUARTIC_DRY_up (VARIABLES, PARAMS, VERTSTRUC, CONSTANTS, sunlit,cntspecies);
        dHcan_dry = nan;
    end
    
    
    if (canstorheat)
        [Tl_wet_lo] = LEB_QUARTIC_WET_CHEAT_lo (VARIABLES, PARAMS, VERTSTRUC, CONSTANTS, sunlit,cntspecies);
    else
        [Tl_wet_lo] = LEB_QUARTIC_WET_lo (VARIABLES, PARAMS, VERTSTRUC, CONSTANTS, sunlit,cntspecies);
        dHcan_wet = nan;
    end
    
    
    %      Tl_lo(vindswet) = Tl_dry(vindswet).*dryfrac(vindswet) + Tl_wet_lo(vindswet).*wetfrac(vindswet);
    %      Tl_lo(nvindswet) = Tl_dry(nvindswet).*dryfrac(nvindswet);
    %
    %      Tl_up(vindswet) = Tl_dry_up(vindswet).*dryfrac(vindswet) + Tl_wet(vindswet).*wetfrac(vindswet);
    %      Tl_up(nvindswet) = Tl_dry_up(nvindswet).*dryfrac(nvindswet);
    ...
    Tl_lo(vindswet) = Tl_wet_lo(vindswet).*wetfrac(vindswet) + Tl_dry(vindswet).*dryfrac(vindswet);
    Tl_lo(nvindswet) = Tl_dry(nvindswet).*dryfrac(nvindswet);
    
    Tl_up(vindswet) = Tl_dry_up(vindswet).*dryfrac(vindswet) + Tl_wet(vindswet).*wetfrac(vindswet);
    Tl_up(nvindswet) = Tl_dry(nvindswet).*dryfrac(nvindswet);
...
%VARIABLES.Respiration = Respiration;
% 
% if VARIABLES.timestep==3735 && sunlit==1
%     PPPPP;
% end
